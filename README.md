...
2. **Чистота** - файлы управляются через симлинки в XDG директории
3. **Откат** - можно откатиться к предыдущей конфигурации
4. **Портативность** - конфигурация легко переносится между системами
5. **Разделение** - пользовательские настройки отделены от системных
6. **Расширяемость** - наш модуль расширяет встроенный, а не заменяет его
