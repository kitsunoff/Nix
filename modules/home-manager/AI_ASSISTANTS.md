...
2. **Независимая настройка** - каждый инструмент можно включить/выключить отдельно
3. **Специфические настройки** - для каждого инструмента свои дополнительные опции
...
2. **Агенты** - управляются через Nix:
   ```bash
...
2. Добавить конфигурацию в `config` блок
3. Всё! Агенты автоматически подхватятся
...
2. Убедитесь, что `agentsPath` указывает на существующую директорию
3. Проверьте логи: `journalctl --user -u home-manager-*`
